import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:injectable/injectable.dart';
import 'package:kortobaa_ecommerce/core/presentation/blocs/base_states/base_state.dart';
import 'package:kortobaa_ecommerce/favorite/domain/usecases/add_product_to_favorite_use_case.dart';
import 'package:kortobaa_ecommerce/home/domain/entities/product.dart';

@lazySingleton
class AddProductToFavoriteCubit extends Cubit<BaseState<int>> {
  final AddProductToFavoriteUseCase _addProductToFavoriteUseCase;
  AddProductToFavoriteCubit(this._addProductToFavoriteUseCase)
      : super(const BaseState());

  void addProductToCart(Product product) async {
    emit(state.setInProgressState());

    final params = AddProductToFavoriteUseCaseParams(product);
    final result = await _addProductToFavoriteUseCase.call(params);

    result.fold(
      (failure) => emit(state.setFailureState(failure)),
      (data) => emit(state.setSuccessState(data)),
    );
  }
}
